  ...
  SELECT
    iif(
      ...
      iif(
        ...
        '', -- Toutes les formalités accomplies → dossier complet
    ...
    iif(
      ...
      iif(
        ...
        iif(
          ...
          iif(
            (SELECT COUNT(*) FROM Remplir WHERE Remplir.IdPersonne = Personne.IdPersonne) = 0,
            'secondary',  -- Jamais venu + rien
            'purple'      -- Jamais venu + quelques formalités
          ),
          iif(
            (SELECT COUNT(*) FROM Remplir WHERE Remplir.IdPersonne = Personne.IdPersonne) = 0,
            'orange',     -- Venu + aucune formalité
            'blue'        -- Venu + formalités incomplètes
          )
        )
      )
    ) AS couleur,
